...
export async function importRssFeed(manualCategory = null) {
    ...
    try {
        ...
        // Clear old breaking news flags before importing new ones
        const breakingQuery = query(collection(db, 'articles'), where('isBreaking', '==', true));
        const breakingSnap = await getDocs(breakingQuery);
        const { updateDoc, doc } = await import('firebase/firestore');
        for (const docSnap of breakingSnap.docs) {
            await updateDoc(doc(db, 'articles', docSnap.id), { isBreaking: false });
        }
        console.log(`Cleared ${breakingSnap.size} old breaking news flags`);
        ...
        if (feedsSnapshot.empty) {
            ...
            // Fix existing Nairobi Wire categorization in DB if it's currently 'politics'
            for (const f of availableFeeds) {
                if (f.name === 'Nairobi Wire' && f.category !== 'news') {
                    console.log('Updating Nairobi Wire category to news in DB...');
                    f.category = 'news';
                    import('firebase/firestore').then(({ updateDoc, doc }) => {
                        updateDoc(doc(db, 'rssFeeds', f.id), { category: 'news' }).catch(console.error);
                    }).catch(console.error);
                }
            }
        }
            ...
            try {
                ...
                for (const item of itemsToProcess) {
                    ...
                    try {
                        ...
                        const allowNoImage = ['opinion', 'politics', 'world', 'news'].includes(feedConfig.category);
                        ...
                        // Mark the first successfully imported article as breaking news
                        const isFirstImport = stats.imported === 0;
                        ...
                        const articleData = {
                            ...
                            isRssImport: true,
                            isBreaking: isFirstImport // Mark first imported article as breaking news
                        };
